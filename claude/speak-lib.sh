...
# Markers and saved replies outlive the consoles that made them — closing a tab
...
# actually wanted to hear. The command leaves this mark; the Stop hook takes it
# and skips the turn.
#
...
speak_say_file() {
    ...
    # stops that optimisation.
    {
        ...
        if nohup "$SPEAK_PIPER_PY" -m piper -m "$onnx" --length-scale "$speed" \
            ...
            # Above 128 means a signal, i.e. someone asked us to stop — that is not
